  ...
  const thongTinCaNhan = await page2.evaluate(() => {
    try {
      ...
      return {
        ...
        lop: lop.split(' - Ngành: ')[0],
        nganh: lop.split(' - Ngành: ')[1].split(' - Khoa: ')[0],
        khoa: lop.split(' - Ngành: ')[1].split(' - Khoa: ')[1],
      };
    } catch (err) {
      return null;
    }
...
const fullCrawl = async () => {
  for (let i = INITIAL_STUDENT_ID; i < INITIAL_STUDENT_ID + 600; i++) {
    await getData(i);
  }
...
const removeStudent = mssv => {
  fs.readFile(FILE_DATA_PATH, 'utf-8', (err, data) => {
    if (err) console.error(err);
      ...
      let students = JSON.parse(data);
      students = students.filter(st => st.mssv !== mssv);
      fs.writeFile(FILE_DATA_PATH, JSON.stringify(students), err => {
        err && console.log(err);
      });
    }
...
rl.question(
  ...
  answer => {
    ...
    if (answer === '5') removeStudent('3118420341');
